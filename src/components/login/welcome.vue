					...
					<div class="login-panel">
						...
						<div class="content">
							...
							<div class="remember-me">
								<input id="remember" type="checkbox" class="login-checkbox">
...
<script>
	...
	export default {
		...
		methods: {
			change(index) {
				if(index == 1) {
					document.getElementById("title_0").className = "";
					document.getElementById("title_1").className = "current";
					this.role = 1;
				} else {
					document.getElementById("title_1").className = "";
					document.getElementById("title_0").className = "current";
					this.role = 0;
				}
			},
			async login() {
				...
				this.ajaxLogin({ //统计
					...
					/*获取token*/
					$.ajax({
						url: this.CommonUrl + 'authorization',
						type: 'post',
						data: this.user,
						success: toeknData => {
							this.setItem('logindata', toeknData);
							this.setItem('token', toeknData.data);
							if(this.role == 0) {
								this.$router.push({
									path: "/map"
								});
							...
							this.$router.options.routes = routers;
								this.$router.replace('system/staff')
								this.$message({
									message: '登录成功',
									type: 'success'
								});
										...
										if(!data.length) { //如果没有配置角色
											...
											if(data[0].children[0].children.length) { //默认走菜单的第一个路径
												...
												this.AssmmenuList.push({
													...
													redirect: data[0].children[0].children[0].url
												})
												this.$router.push(data[0].children[0].children[0].url);
											} else {
												/*增加页面重定向*/
												this.AssmmenuList.push({
													path: '*',
													hiddenMenu: true, //隐藏菜单栏
													redirect: data[0].children[0].url
												})
												this.$router.push(data[0].children[0].url);

												console.log(data[0].children[0].url)
											}
											this.$router.replace(data[0].url + '/' + data[0].children[0].url)
											this.$message({
												message: '登录成功',
												type: 'success'
											});
